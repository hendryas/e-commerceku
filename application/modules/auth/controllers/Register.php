...
class Register extends CI_Controller
{
    ...
    {
        $this->form_validation->set_rules('nama', 'Nama', 'required|trim', [ //bener
            'required' => 'Nama tidak boleh kosong.'
        ]);
        $this->form_validation->set_rules('email', 'Email', 'required|trim|valid_email', [ //bener
            'required' => 'Email tidak boleh kosong.',
        ...
        $this->form_validation->set_rules('password', 'Password', 'required|trim', [ //bener
            'required' => 'Password tidak boleh kosong.'
        ]);
        ...
        if ($this->form_validation->run() == false) {
            ...
            $cekEmail = $this->authModel->cekEmailAuth($email)->result();
            $isEmail = count($cekEmail);

            if ($isEmail > 0) {
                $this->session->set_flashdata('message', '<div class="alert alert-danger text-center" role="alert">
                <strong>Email sudah pernah dibuat!</strong> <br> Silahkan gunakan email yang lain!</div>');
                redirect('auth/login');
            } else {
                $data = [
                    'name' => $nama,
